...
var http_handler = function(request, res) {
  ...
  // if file doesn't exist, try to set filename to 
  // node_module js.
  if (!fs.existsSync(filename)) {
    try {
      var filename = require.resolve(uri.slice(1));
    } catch (err) {
    }
  }
...
function browserify_od(filepath, callback) {
  var b = browserify();
  b.add(filepath);
...
function static_js_handler(filename, file, res) {
  if (is_common_js(file)) {
    ...
    browserify_od(filename, function(err, src) {
      if (src) {
        res.write(src);
      } else {
